...
//
// ReserveCalifornia's API sits behind a CloudFront WAF that blocks a subset
// of datacenter/cloud IPs by reputation (403 "Request blocked"), independent
// of rate or headers (measured 2026-07-08: ~40% of Cloud Run IPs work). It
// goes through the shared proxy pool like every other provider; the pool's
// per-(IP, target) escalating backoff learns which egress IPs the WAF allows
// and routes around the blocked ones.
type ReserveCalifornia struct {
	client *http.Client
}

func NewReserveCalifornia() *ReserveCalifornia {
	return &ReserveCalifornia{client: httpx.Default()}
}
